...
expected_outputs=(
  ./cafe/test/CafeTest.bo
  ./cafe/test/mkCafeTest.v
)
BSC \
  -verilog \
  $(path_dir_flags ./cafe/test) \
  -g mkCafeTest \
  ${source_files[@]}
...
expected_outputs=(
  ...
  ./directc_mkCafeTest.so
)
extra_outputs=(
  ./cafe/cafe.o
  ./cafe/test/vpi_startup_array.{c,o}
  ./cafe/test/vpi_wrapper_cafe.o
)
BSC \
  -verilog \
  $(path_dir_flags ./cafe/test) \
  -o ./cafe/test/CafeTest.exe \
  -e mkCafeTest \
  ${source_files[@]}
# ^ fails because it can't find ./test/vpi_wrapper_cafe.{c,h}
